...
/data/software/rdfpro/rdfpro @read $INPUT \
       @reformattime @esoreasoner /data/instances/ksdemo/etc/ESO_Version2.owl { \
            @transform " " , \
            @groovy -p /data/scripts/groovy/gen_definedBy.groovy , \
            @groovy -p /data/scripts/groovy/gen_time_label.groovy } \
       @unique @write $OUTPUT
